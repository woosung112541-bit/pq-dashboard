...
# 💡 [핵심] 파이썬을 이용한 환산 데이터 선행 계산기
def calculate_fact_data(master_df):
    if master_df.empty: return "DB 데이터 없음"
    fact_summary = "=== [파이썬 시스템에서 수학적으로 완벽히 사전 계산한 팩트 데이터] ===\n"
    fact_summary += "※ AI는 아래의 수치를 절대 임의로 변경하거나 재계산하지 말고, 오직 [세부평가기준]의 어느 배점 구간에 해당하는지만 찾아서 점수를 매길 것.\n\n"
    ...
    if name_col:
        for name, group in master_df.groupby(name_col):
            fact_summary += f"▶ 기술인: {name}\n"
            
            # 여기서 가중치를 임의로 100%, 80% 등으로 적용한 가상의 정밀 계산 로직을 파이썬으로 구현
            # 실제로는 DB 컬럼 구조에 맞춰 더 정교하게 짜야 함 (현재는 예시 로직)
            total_days = 0
            total_count = 0
            total_amount = 0
            for _, row in group.iterrows():
                # 임의 가중치 적용 (실제 DB 구조에 맞게 커스텀 필요)
                weight = 1.0 if '교량' in str(row.values) or '터널' in str(row.values) else 0.8
                
                # 일수 계산
                days = 0
                for v in row.values:
                    if isinstance(v, (int, float)) and v > 1000 and v < 10000: # 대략 일수 컬럼이라 가정
                        days = v; break
                ...
                amt = 0
                for v in row.values:
                    if isinstance(v, (int, float)) and v > 10000: # 금액 컬럼이라 가정
                        amt = v; break
            ...
            fact_summary += f" - 환산 경력(년): {years:.2f}년\n"
            fact_summary += f" - 환산 실적 건수: {total_count:.1f}건\n"
            fact_summary += f" - 환산 실적 금액: {total_amount:,.0f} 원\n\n"
    else:
        fact_summary += master_df.to_string(index=False)
...
class PQScoringEngine:
    ...
    def run_ai_dreamteam_optimizer(self, notice_text, excel_bytes, semi_fixed):
        ...
        fact_data_summary = calculate_fact_data(master_db)
